...
"""def main(data_dir):
    data_dir = os.path.join(data_dir, "Color")
...
def main(data_dir):
    ...
    for imgs_list, imgs_dir in zip(imgs_path_list, dir_list):
        for img_path in imgs_list:  
            ...
            for i in range(unique_colors_np.shape[0]):
                color = unique_colors_np[i]
                count = counts_np[i]
                if tuple(color) in lab:
                    lab[tuple(color)] += count
                else:
                    lab[tuple(color)] = count
    ...
    lab = sorted(lab.items())
    counts = [lab[i][1] for i in range(len(lab))]
    print(counts)
    lab = [list(lab[i][0]) for i in range(len(lab))]
    print(lab)
